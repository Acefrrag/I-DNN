...
This script was used to support the testbench of the architecture.

Inputs:
    neuron_folder: String contain the name of the folder where the neuron
    parameters are located. The containing files are:
        -weight_file.txt
        -bias_file.txt
        -input_data_file.txt
    neuronweight_IntWidth: Integer Part of weights
    neuroninut_IntWidth: Integer Part of input/output of neurons
    neuronbias_IntWidth: Integer Part of bias
Output:
    neuron_ws: Neuron's weighted sum (The ReLU sigmoid is not applied to the ouput)'
    
    
This script is deprecated. It is based on a older version of numpy, which does not
support data read with \n (new line) character delimiter.
